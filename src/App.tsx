        ...
        {!show ? (
          ...
          <motion.div key="main" initial={{ opacity: 0 }} animate={{ opacity: 1 }} style={{ width: '100%', height: '100%' }}>
            ...
            <div style={{ 
              ...
              <div style={{ display: 'flex', flexDirectionimport React, { useState, useEffect, useRef, Suspense, useMemo } from 'react';
import { Canvas, useFrame, useLoader } from '@react-three/fiber';
import { motion, AnimatePresence } from 'framer-motion';
import * as THREE from 'three';
import { hapticFeedback } from '@telegram-apps/sdk';
...
// --- WebGL: Иммерсивный фон с мягкими краями ---
const Scene = ({ isMobile }: { isMobile: boolean }) => {
  const meshRef = useRef<THREE.Mesh>(null!);
  const texture = useLoader(THREE.TextureLoader, '/sector88.jpg');
  ...
  const shaderData = useMemo(() => ({
    ...
    fragmentShader: `
      ...
      void main() {
        vec4 tex = texture2D(uTexture, vUv);
        // Эффект SyncGo: затухание по краям (двусторонний туман)
        float edge = smoothstep(0.0, 0.15, vUv.x) * smoothstep(1.0, 0.85, vUv.x) *
                     smoothstep(0.0, 0.15, vUv.y) * smoothstep(1.0, 0.85, vUv.y);
        gl_FragColor = vec4(tex.rgb, tex.a * edge);
      }
  ...
  return (
    <mesh ref={meshRef}>
      <planeGeometry args={[isMobile ? 3.8 : 7, isMobile ? 6.5 : 3.8]} />
      <shaderMaterial args={[shaderData]} transparent depthWrite={false} />
    </mesh>
  );
};
...
export default function App() {
  ...
  const menuItems: IMenuItem[] = [
    { id: 'sys', label: 'ACCESS_CORE', content: 'SYSTEM_STABLE. NEURAL_LINK: ACTIVE.' },
    { id: 'prj', label: 'PROJECTS', content: 'SCANNING_ASSETS... [PULSAR_V2], [CYBER_SHELL].' },
    { id: 'net', label: 'NETWORK', content: 'UPLINK: @VISITVITALIK_HUE' },
  ];
  ...
  // Логика загрузки с Haptic Feedback
  useEffect(() => {
  ...
  return (
    <div style={{ 
      ...
      position: 'relative', overflow: 'hidden', color: '#fff', fontFamily: 'monospace' 
    }}>
      <AnimatePresence mode="wait">
        {!show ? (
          <motion.div 
            key="loader" exit={{ opacity: 0, scale: 1.1 }}
            style={{ height: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', zIndex: 2000 }}
          >
            <div 
              onPointerDown={(e) => { (e.target as HTMLElement).releasePointerCapture(e.pointerId); setHold(true); }}
              onPointerUp={() => setHold(false)}
              onPointerLeave={() => setHold(false)}
              style={{ 
                width: 140, height: 140, border: '2px solid #00f2ff', borderRadius: '50%', 
                display: 'flex', alignItems: 'center', justifyContent: 'center', cursor: